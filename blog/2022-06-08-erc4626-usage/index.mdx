...
end of the article you will be able to do that yourself, because we are open-sourcing Apollo along with this article.
...
So to filter out the correct events, we can call `asset` on the contract for which the event was emitted.
